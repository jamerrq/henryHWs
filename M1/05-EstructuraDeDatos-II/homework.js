...
/**
 * Solución Tarea # 05 Módulo 1 Henry Full Stack Course FT
 *    AUTOR: Jamer José Rebolledo Quiroz
 *   GitHub: https://github.com/jamerrq
 * Linkedin: https://linkedin.com/in/jamerrq
 */

/* EJERCICIO 1 Implementar la clase LinkedList, definiendo los siguientes
métodos:
  - add: agrega un nuevo nodo al final de la lista;
  - remove: elimina el último nodo de la lista y retorna su valor (tener en
    cuenta el caso particular de una lista de un solo nodo y de una lista
    vacía);
  - search: recibe un parámetro y lo busca dentro de la lista, con una
    ...
    buscamos un nodo cuyo valor, al ser pasado como parámetro del callback,
    retorne true. EJEMPLO search(3) busca un nodo cuyo valor sea 3;
    search(isEven), donde isEven es una función que retorna true cuando recibe
    por parámetro un número par, busca un nodo cuyo valor sea un número par. En
    caso de que la búsqueda no arroje resultados, search debe retornar null.
*/
...
class LinkedList {
    ...
    add(data) {
        ...
        // Uso un objeto temporal para ir recorriendo mi lista empiezo en la
        // cabeza
        let temp = this.head;
    ...
    remove() {
        ...
        // Para los demás casos, usaremos una variable temporal para recorrer la
        // lista, también una variable last para almacenar el valor a remover
        let temp = this.head;
        let last = null;

        // Caso que sólo exista la cabeza
        if (this.size == 1) {
            // Almacenamos el valor de la cabeza en una variable temporal porque
            // luego retornaremos este valor
            last = temp.value;
            // Reasignamos la propiedad cabeza como null, esto es equivalente a
            // resetear esta propiedad de fábrica.
            this.head = null;
        }
        ...
        // Caso que exista la cabeza y hayan más nodos conectados
            ...
            // Se desconecta el último nodo del penúltimo, esto es equivalente a
    ...
    search(param) {
        ...
        while (temp) {
            ...
            // Si el tipo de parámetro pasado, no es una función, pregunto si el
            // valor de mi nodo actual coinicide con el parámetro pasado
            ...
            // evaluo el retorno de esa función cuando la invoco con el valor de
            // mi nodo actual
            else {
                // En caso que la función me retorne true, devuelvo el valor
                // encontrado
...
/* EJERCICIO 2 Implementar la clase HashTable. Nuetra tabla hash, internamente,
consta de un arreglo de buckets (slots, contenedores, o casilleros; es decir,
posiciones posibles para almacenar la información), donde guardaremos datos en
formato clave-valor (por ejemplo, {instructora: 'Ani'}). Para este ejercicio, la
tabla debe tener 35 buckets (numBuckets = 35). (Luego de haber pasado todos los
tests, a modo de ejercicio adicional, pueden modificar un poco la clase para que
...
La clase debe tener los siguientes métodos:
  - hash: función hasheadora que determina en qué bucket se almacenará un dato.
    Recibe un input alfabético, suma el código numérico de cada caracter del
    input (investigar el método charCodeAt de los strings) y calcula el módulo
    de ese número total por la cantidad de buckets; de esta manera determina la
  ...
  - set: recibe el conjunto clave valor (como dos parámetros distintos), hashea
    la clave invocando al método hash, y almacena todo el conjunto en el bucket
    correcto.
...
puedo chequear, con hasKey, si ya hay algo en la tabla con el nombre
'instructora'; luego, invocando set('instructora', 'Ani'), se almacenará el par
...
class HashTable {
    ...
    constructor(numBuckets = 35) {
        // Se usa como valor por defecto 35, esto permite crear instancias con
        // un número diferente de buckets
    ...
    hasKey(key) {
        ...
        let bucket = this.buckets[index];
        // Devuelvo el resultado directo de preguntar si el bucket tiene la
        // propiedad correspondiente almacenada
