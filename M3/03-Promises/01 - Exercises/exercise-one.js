...
function problemA() {
    ...
    // promise version
    // Tu código acá:
    exerciseUtils.promisifiedReadFile("poem-one/stanza-02.txt").then(async (stanza2) => {
        exerciseUtils.blue(stanza2);
...
function problemB() {
    ...
    // promise version
    // Tu código acá:
    exerciseUtils.promisifiedReadFile("poem-one/wrong-file-name.txt").then((stanza4) => {
...
function problemC() {
    ...
    // promise version
    // Tu código acá:
    exerciseUtils.promisifiedReadFile("poem-one/stanza-03.txt").then((stanza3) => {
        exerciseUtils.blue(stanza3);
        return exerciseUtils.promisifiedReadFile("poem-one/wrong-file-name.txt").then((stanza4) => {
            exerciseUtils.blue(stanza4);
        }).catch((err2) => {
            exerciseUtils.magenta(new Error(err2));
        });
    }).catch((err) => {
        exerciseUtils.magenta(new Error(err));
    });
